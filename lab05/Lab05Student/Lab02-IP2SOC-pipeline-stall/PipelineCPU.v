...
module PipelineCPU(
    ...
    output [31:0]   PC_out_IDEX, //执行阶段PC输出
    output [31:0]   PC_out_EXMem, // 访存阶段PC输出
    output [31:0]   PC_out_MemWB , // 写回阶段PC输出
    ...
    output [1023:0] all_regs_data         //所有寄存器数据输出
    );
    ...
    wire [31:0] PC_out_EX , Rs1_out_IDEX , Rs2_out_IDEX, Imm_out_IDEX;
    wire [4:0] Rd_addr_out_IDEX;
    wire [3:0] ALU_control_IDEX;
    wire [1:0] MemtoReg_IDEX, Jump_IDEX;
    wire ALUSrc_B_IDEX , Branch_IDEX , BranchN_IDEX , MemRW_IDEX  , RegWrite_IDEX;
    ...
    wire en_IF , en_IFID , NOP_IFID , NOP_IDEX;

    wire[31:0] PC_imm_out_EXMem, Inst_out_EXMem;
    wire valid_out_EXMem; // Part 'Ex_reg_Mem'
    ...
    Ex_reg_Mem Ex_reg_Mem(
        ...
        .Jump_in_EXMem(Jump_IDEX), // ‘jump’ 好一个错误 
